...
sub_pretext = rng.choice(subjects, 6, replace=False)
sub_train = sorted(
...
for i, arr in tqdm(enumerate(pretext_loader), desc = 'pretext'):
    temp_path = os.path.join(PRETEXT_PATH, 'pretext_' + str(i) + '.npz')
    np.savez(temp_path, arr)
  
for i, arr in tqdm(enumerate(train_loader), desc = 'train'):
    temp_path = os.path.join(TRAIN_PATH, 'train_' + str(i) + '.npz')
    np.savez(temp_path, arr)
    
for i, arr in tqdm(enumerate(test_loader), desc = 'test'):
    temp_path = os.path.join(TEST_PATH, 'test_' + str(i) + '.npz')
    np.savez(temp_path, arr)
