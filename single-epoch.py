...
class RelativePositioningDataset(BaseConcatDataset):
    ...
    def __getitem__(self, index):
        ...
        pos_data = super().__getitem__(pos)[0]
        neg_data= super().__getitem__(neg)[0]

        anc = augment(pos_data)  # (2, 3000)
        pos_data = augment(pos_data)  # (2, 3000)
        neg_data = augment(neg_data)  # (2, 3000)
    
        return anc, pos_data, neg_data
...
def Pretext(
    ...
    for epoch in range(Epoch):
        ...
        for index, (anc, pos, neg, _) in enumerate(
            ...
            pos_features = q_encoder(pos, proj_first=False)  # (B, 128)
...
train_loader = DataLoader(
    splitted["train"], batch_size=BATCH_SIZE, shuffle=True, num_workers=12, persistent_workers = True
)
test_loader = DataLoader(
    splitted["test"], batch_size=BATCH_SIZE, shuffle=False, num_workers=12, persistent_workers = True
)

wandb.init(
    project="multi-epoch",
    notes="triplet loss, taking the average of features",
    save_code=True,
