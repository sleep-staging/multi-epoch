        ...
        def validation_epoch_end(self,outputs):
            ...
            if f1_score > self.best_score:
                ...
                self.val_dict = {"Valid F1": f1_score,
                            ...
                            "Valid Balanced Acc": balanced_accuracy_score(epoch_targets.cpu().numpy(), epoch_preds.argmax(dim=1).cpu().numpy()),
                            "Valid Acc":epoch_acc,
                            "Valid Loss":epoch_loss,
                            "Epoch":self.EPOCH}
                self.val_pd = epoch_preds.cpu().numpy()
                self.val_gt = epoch_targets.cpu().numpy()
